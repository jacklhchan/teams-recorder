...
do {
    ...
    if command == "verify" {
        ...
        let value = try ReleaseManifest(version: args["--version"]!, build: args["--build"]!, minimumAcceptedBuild: args["--minimum-accepted-build"]!, keyID: args["--key-id"]!, provenanceID: args["--provenance-id"]!, zipFilename: zip.lastPathComponent, zipData: Data(contentsOf: zip)); let bytes = try value.canonicalData(); try bytes.write(to: manifestOut, options: .withoutOverwriting); try Data((try ReleaseManifest.sign(bytes, privateKeySeed: seed)).base64EncodedString().appending("\n").utf8).write(to: signatureOut, options: .withoutOverwriting)
