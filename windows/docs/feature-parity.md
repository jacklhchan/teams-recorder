...
| 手動／10 秒測試／Teams 自動擁有權 | manual 不被 auto stop；Teams 自動有 countdown/debounce | manual、test、Teams-automatic 皆進 application lifecycle；自動開始使用明確 opt-in 的本機 WASAPI heuristic；自動停止只讀 exact Teams meeting control，連續 3 次健康缺失才提出，無 Teams API | coordinator/lifecycle/heuristic tests：3 次 active start；meeting control 必須先出現才武裝 stop；silence/probe fault/短暫缺失/rejoin 不 stop；連續 3 次健康缺失或 Teams process 不存在才 stop | 真實 Teams 會議開始、沉默、離開與手動停止 smoke | heuristic 不得聲稱 authoritative meeting state；升級後需重新同意 | Draft PR #11 |
