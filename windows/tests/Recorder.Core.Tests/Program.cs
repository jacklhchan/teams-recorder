...
var tests = new (string Name, Action Run)[]
{
    ...
    ,("process-loopback supports any app and never silently changes source", CaptureSourceChoiceTests.SupportsAnyApplicationAndNeverSilentlyFallsBack)
    ...
    ,("application picker exposes only eligible current-session processes", ApplicationProcessCatalogPolicyTests.ExposesEligibleAppsInCurrentSessionOnly)
    ...
    ,("Windows video metadata removes runtime window identity", MetadataPrivacyTests.VideoMetadataDoesNotPersistRuntimeWindowIdentity)
    ...
    ,("session library includes only published video media", SessionStorageTests.LibraryIncludesOnlyPublishedVideoMedia)
    ,("video publication promotes MP4 with a final M4A fallback", SessionStorageTests.VideoPublicationPromotesMp4AndRetainsM4aFallback)
    ,("video publication preserves M4A when a partial MP4 is invalid", SessionStorageTests.VideoPublicationFallsBackToM4aAndNeverPublishesAnInvalidPartialMp4)
    ,("default MP4 validator rejects structural but undecodable media", SessionStorageTests.DefaultMp4ValidatorRejectsStructuralForgery)
    ,("AT-06 MP4 validator rejects corrupt fixtures", FaultInjectionAcceptanceTests.Mp4ValidatorRejectsCorruptFixtures)
    ,("AT-06 M4A validator rejects corrupt fixtures", FaultInjectionAcceptanceTests.M4aValidatorRejectsCorruptFixtures)
    ,("AT-06 video initialization fault publishes an independent M4A", FaultInjectionAcceptanceTests.VideoInitializationFaultPublishesIndependentM4a)
    ,("AT-06 MP4 writer fault preserves M4A and MP4 evidence", FaultInjectionAcceptanceTests.Mp4WriterFaultPublishesM4aAndRetainsVideoEvidence)
    ,("AT-06 fatal video start retains M4A for startup recovery", FaultInjectionAcceptanceTests.FatalVideoStartFaultRetainsM4aForStartupRecovery)
    ,("AT-06 recovery promotes only valid M4A after video failure", FaultInjectionAcceptanceTests.StorageRecoveryPromotesOnlyValidM4aAfterVideoFailure)
    ,("video recovery promotes only validated MP4 with an M4A fallback", SessionStorageTests.RecoveryPromotesOnlyValidatedMp4WithAnM4aFallback)
    ,("video recovery sanitizes existing metadata without touching media", SessionStorageTests.RecoverySanitizesExistingVideoMetadataWithoutChangingMedia)
    ,("session recovery is idempotent and never clobbers", SessionStorageTests.RecoveryIsIdempotentAndNeverClobbers)
    ,("crash recovery uses canonical paths, a durable journal, and an exclusive lock", CrashSafeRecoveryTests.CanonicalPathsJournalAndExclusiveLockAreDurable)
    ,("fMP4 recovery scanner selects only complete durable fragments", CrashSafeRecoveryTests.ScannerSelectsOnlyCompleteDurableFragments)
    ,("fMP4 recovery scanner handles extended and overflowing boxes", CrashSafeRecoveryTests.ScannerHandlesExtendedAndOverflowBoxesFailClosed)
    ...
    ,("dynamic video rejects HWND reuse, resize, close, and stale callbacks", DynamicWindowVideoCaptureAcceptanceTests.HwndReuseResizeCloseAndStaleCallbacksNeverLeakPriorPixels)
    ,("delayed first video frame keeps a black prefix and continuous audio", DynamicWindowVideoCaptureAcceptanceTests.DelayedFirstRealFrameKeepsOneWriterBlackPrefixAndAudio)
    ,("dynamic video off-on-off-on keeps one writer, black gaps, and audio", DynamicWindowVideoCaptureAcceptanceTests.OffOnOffOnUsesOneWriterBlackGapsAndContinuousAudio)
    ,("dynamic recording without an enabled target publishes audio-only MP4", DynamicWindowVideoCaptureAcceptanceTests.NeverEnabledTargetPublishesAudioOnlySafetyMp4)
    ,("a disappearing initial video target publishes audio-only MP4", DynamicWindowVideoCaptureAcceptanceTests.DisappearingInitialTargetPublishesAudioOnlySafetyMp4)
    ,("an enabled but black-only video target publishes audio-safe media and retains evidence", DynamicWindowVideoCaptureAcceptanceTests.EnabledButBlackOnlyTargetPublishesAudioSafetyMp4)
    ,("a muxed exact-window frame publishes video media", DynamicWindowVideoCaptureAcceptanceTests.CapturedWindowFramePublishesVideo)
    ,("an exact-window frame publishes the A/V MP4", DynamicWindowVideoCaptureAcceptanceTests.CapturedWindowFramePublishesVideo)
    ,("local Teams detection never recognizes unrelated windows", TeamsLocalIntegrationTests.OtherWindowsAreNeverRecognizedAsTeamsMeetings)
    ,("local Teams detection rejects PID and HWND reuse until reconfirmed", TeamsLocalIntegrationTests.PidAndHwndReuseEndsTheOldMeetingBeforeReconfirmation)
    ,("local Teams WinEvent jitter is debounced", TeamsLocalIntegrationTests.WinEventJitterSchedulesOneRefreshAndOneTransition)
    ,("stale or rejected Teams UI Automation never changes recorder mute", TeamsLocalIntegrationTests.StaleOrRejectedUiaNeverChangesRecorderMute)
    ,("local Teams integration payloads are bounded and privacy safe", TeamsLocalIntegrationTests.LocalIntegrationPayloadsAreBoundedAndPrivacySafe)
    ...
    ,("Teams mute remains independent from Recorder manual mute", InputMuteTests.TeamsMuteIsIndependentAndNeverClearsManualMute)
    ,("exact Teams mute action names map to Recorder mute", TeamsMuteFollowTests.ExactTeamsActionNamesMapToRecorderMute)
    ,("missing Teams controls stay unmuted while ambiguity fails closed", TeamsMuteFollowTests.MissingControlMeansNotInCallButAmbiguityFailsClosed)
    ...
    ,("local diagnostic export removes transient window identity", LocalDiagnosticLogTests.RedactsTransientWindowIdentity)
    ,("crash marker is atomic bounded and privacy safe", RecorderCrashMarkerTests.MarkerIsAtomicBoundedAndPrivacySafe)
    ...
    ,("AI provider profiles and API keys remain independent", OpenAICompatibleProviderTests.ProviderProfilesAndKeysRemainIndependent)
    ...
    ,("manual transcription import creates owned playable media", ManualTranscriptionImporterTests.ImportedAudioBecomesOwnedPlayableTranscriptionMedia)
    ,("manual transcription imports are unique and explicitly gated", ManualTranscriptionImporterTests.ImportsAreUniqueAndRequireExplicitImportedMetadata)
    ...
    ,("managed MP4 ASR export produces chronological complete bounded chunks", MediaFoundationRecordingSessionAsrChunkSourceTests.ManagedMp4ProducesChronologicalCompleteBoundedChunks)
    ,("managed MP4 ASR export enforces duration and byte caps", MediaFoundationRecordingSessionAsrChunkSourceTests.EnforcesPerChunkDurationAndByteCapsBeforeYielding)
    ,("managed MP4 ASR export cancellation removes staging files", MediaFoundationRecordingSessionAsrChunkSourceTests.CancellationDeletesBoundedStagingFiles)
    ,("corrupt managed MP4 ASR export fails closed and cleans staging", MediaFoundationRecordingSessionAsrChunkSourceTests.CorruptManagedMp4FailsClosedAndCleansStaging)
    ,("isolated MF ASR startup timeout kills its worker and cleans staging", IsolatedMediaFoundationAsrWorkerTests.BlockedWorkerOpenTimesOutStopsAndCleansStaging)
    ,("isolated MF ASR read timeout kills its worker and cleans staging", IsolatedMediaFoundationAsrWorkerTests.BlockedWorkerReadTimesOutStopsAndCleansStaging)
    ,("isolated MF ASR cancellation is prompt while a worker read is blocked", IsolatedMediaFoundationAsrWorkerTests.CancellationDuringBlockedWorkerReadReturnsPromptlyAndCleansStaging)
    ,("isolated MF ASR cleanup is bounded when worker shutdown blocks", IsolatedMediaFoundationAsrWorkerTests.BlockedWorkerStopCannotHoldCleanupHostage)
    ,("isolated MF ASR acknowledges a chunk only after consumer advance", IsolatedMediaFoundationAsrWorkerTests.WorkerChunkIsAcknowledgedOnlyAfterItsConsumerAdvances)
    ,("meeting intelligence bounds a maximum transcript to seventy-one requests", MeetingIntelligenceParityTests.BoundedPipelineHandlesMaximumTranscriptInSeventyOneRequests)
    ,("meeting intelligence treats transcripts as untrusted provider data", MeetingIntelligenceParityTests.ClientTreatsTranscriptAsUntrustedAndRejectsUnsafeOutput)
    ,("meeting intelligence fences transcript revisions and protects manual titles", MeetingIntelligenceParityTests.CoordinatorFencesTranscriptRevisionAndProtectsManualTitle)
    ,("automatic meeting intelligence requires exact model availability", MeetingIntelligenceParityTests.AutomaticGenerationRequiresExactAvailabilityButManualMayBypass)
    ,("meeting intelligence startup marks abandoned generation interrupted", MeetingIntelligenceParityTests.StateRecoveryMarksAbandonedGenerationInterrupted)
    ,("long ASR uses complete chunks, immutable settings, and rolling context", LongRecordingAsrOrchestrationTests.CompleteChunksUseRollingContextAndOneImmutableSnapshot)
    ,("long ASR rejects an invalid planned chunk before upload", LongRecordingAsrOrchestrationTests.InvalidPlannedChunkFailsBeforeAnyUpload)
};
