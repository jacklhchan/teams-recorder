...
public enum ReleaseManifestError: Int, Error, Equatable, Sendable { case usage = 64, malformed = 65, unsafeFile = 66, unsupported = 67, invalidSignatureOrDigest = 68, rollback = 69, execution = 70 }
...
public struct ReleaseManifestKeyring: Codable, Equatable, Sendable {
    ...
    func activeKey(_ id: String) throws -> ReleaseManifestKey { let matches = entries.filter { $0.keyID == id }; guard schemaVersion == 1, matches.count == 1, let key = matches.first, key.status == .active, ReleaseManifest.validDecimal(key.minimumAcceptedBuild), ReleaseManifest.validToken(key.keyID), Data(base64Encoded: key.publicKeyBase64)?.count == 32 else { throw ReleaseManifestError.unsupported }; return key }
}
...
public struct ReleaseManifest: Codable, Equatable, Sendable {
    ...
    public let artifact: Artifact; public let build: String; public let gitCommit: String; public let keyID: String; public let minimumAcceptedBuild: String; public let provenanceID: String; public let schemaVersion: Int; public let version: String
    public init(version: String, build: String, minimumAcceptedBuild: String, gitCommit: String, keyID: String, provenanceID: String, zipFilename: String, zipData: Data) throws { self.artifact = Artifact(sha256: Self.digest(zipData), zipFilename: zipFilename); self.build = build; self.gitCommit = gitCommit; self.keyID = keyID; self.minimumAcceptedBuild = minimumAcceptedBuild; self.provenanceID = provenanceID; self.schemaVersion = 1; self.version = version; try validate() }
    public func canonicalData() throws -> Data { try validate(); return Data("{\"artifact\":{\"sha256\":\"\(artifact.sha256)\",\"zipFilename\":\"\(artifact.zipFilename)\"},\"build\":\"\(build)\",\"gitCommit\":\"\(gitCommit)\",\"keyID\":\"\(keyID)\",\"minimumAcceptedBuild\":\"\(minimumAcceptedBuild)\",\"provenanceID\":\"\(provenanceID)\",\"schemaVersion\":1,\"version\":\"\(version)\"}\n".utf8) }
    ...
    public static func verify(manifestData: Data, signature: Data, zipData: Data, zipFilename: String, keyring: ReleaseManifestKeyring, minimumBuild: String) throws { let manifest = try decodeCanonical(manifestData); let key = try keyring.activeKey(manifest.keyID); guard validDecimal(minimumBuild), signature.count == 64, let keyData = Data(base64Encoded: key.publicKeyBase64) else { throw ReleaseManifestError.invalidSignatureOrDigest }; let publicKey = try Curve25519.Signing.PublicKey(rawRepresentation: keyData); guard publicKey.isValidSignature(signature, for: manifestData), zipFilename == manifest.artifact.zipFilename, digest(zipData) == manifest.artifact.sha256 else { throw ReleaseManifestError.invalidSignatureOrDigest }; let floor = [manifest.minimumAcceptedBuild, key.minimumAcceptedBuild, minimumBuild].max { compareDecimal($0, $1) < 0 }!; guard compareDecimal(manifest.build, floor) >= 0 else { throw ReleaseManifestError.rollback } }
    static func validToken(_ value: String) -> Bool { !value.isEmpty && value.unicodeScalars.allSatisfy { $0.value >= 33 && $0.value <= 126 && $0 != "\\" && $0 != "/" && $0 != "\"" } }
