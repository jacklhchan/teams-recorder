...
| Teams 靜音整合預覽 | 已有本機 Teams Third-party App API WebSocket client、DPAPI token store、pairing、接收迴圈啟動後的一次 `query-state` 與 fail-closed 行為；單一環境已完成配對及 meeting presence 實測。初始「未靜音」快照不會自動開啟本機錄音麥克風。 | Teams UI mute/unmute 未可靠推送後續狀態；不得宣稱 Teams API 已普遍可用、mute sync／控制可用或可靠。單次狀態查詢不取代後續 mute 推送。 |
...
- 同一次驗證中，Teams UI 的 mute/unmute 均未可靠推送後續 mute-state；Recorder 在接收迴圈已啟動後只發出一次 `query-state` 取得目前會議狀態，並保留已配對連線的後續推送。單一「未靜音」快照不會自動開啟本機錄音麥克風。
