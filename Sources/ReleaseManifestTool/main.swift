...
func parse(_ arguments: [String], names: Set<String>) throws -> [String: String] {
    ...
    while index < arguments.count { let name = arguments[index]; guard names.contains(name), index + 1 < arguments.count, result[name] == nil, !arguments[index + 1].hasPrefix("--") else { throw ReleaseManifestError.usage }; result[name] = arguments[index + 1]; index += 2 }
    guard result.count == names.count else { throw ReleaseManifestError.usage }; return result
}
...
do {
    let all = Array(CommandLine.arguments.dropFirst()); guard let command = all.first else { throw ReleaseManifestError.usage }; let values = Array(all.dropFirst())
    if command == "verify" {
        ...
        try ReleaseManifest.verify(manifestData: Data(contentsOf: manifest), signature: sig, zipData: Data(contentsOf: zip), zipFilename: zip.lastPathComponent, keyring: try JSONDecoder().decode(ReleaseManifestKeyring.self, from: Data(contentsOf: keyring)), minimumBuild: args["--minimum-build"]!); print("release_manifest_verified")
    ...
    } else { throw ReleaseManifestError.usage }
