...
_Static_assert(offsetof(RecorderNativeStats, render_impulse_candidate_frames) == RECORDER_NATIVE_STATS_V3_SIZE, "impulse diagnostics must be additive");
_Static_assert(RECORDER_NATIVE_STATS_V4_SIZE == 272u, "v4 stats layout changed");
...
int main(void) {
    ...
    if (!expect(recorder_native_start(bridge) == RECORDER_NATIVE_INVALID_ARGUMENT,
        ...
        !expect(stats.struct_size == sizeof(stats) && stats.packets == 0U && stats.output_frames == 0U &&
                stats.primary_level_peak == 0.0F && stats.primary_level_rms == 0.0F &&
