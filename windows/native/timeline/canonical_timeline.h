...
struct SourceCounters {
    ...
    // Diagnostics stay internal to the native timeline.  They deliberately do
    // not change the published native-bridge statistics ABI.
...
private:
    struct State {
        ...
        // A source can begin with AUDCLNT_BUFFERFLAGS_TIMESTAMP_ERROR. In
        // that case no QPC/device baseline may be trusted until a later
        // packet supplies one.
        bool needs_reliable_reanchor = false;
        std::uint64_t first_device_position = 0;
        std::uint64_t first_qpc_frame = 0;
        std::uint64_t qpc_bias_frames = 0;
