...
var tests = new (string Name, Action Run)[]
{
    ...
    ,("local Teams detection debounces audio and emits one start", TeamsLocalMeetingDetectorTests.DebouncesActiveAudioAndEmitsOneStartSignal)
    ,("local Teams detection rejects transient audio", TeamsLocalMeetingDetectorTests.TransientAudioDoesNotTriggerAndNegativeEvidenceResetsCandidate)
    ,("local Teams detection never treats lost evidence as meeting end or Teams mute", TeamsLocalMeetingDetectorTests.LostEvidenceNeverClaimsMeetingEndOrChangesTeamsMute)
    ,("local Teams detection fails closed and requires explicit reset", TeamsLocalMeetingDetectorTests.ProbeFailureFailsClosedUntilExplicitReset)
    ,("local Teams heuristic requires explicit user opt-in", TeamsLocalMeetingDetectorTests.DefaultPolicyOnlyReportsLikelyMeetingUntilUserOptsIn)
    ,("local Teams host polls only after opt-in on degraded transport", TeamsLocalMeetingDetectorTests.HostPollsOnlyForOptedInDegradedTransport)
    ,("local Teams host rejects overlap and never forwards evidence loss", TeamsLocalMeetingDetectorTests.HostRejectsOverlappingPollsAndNeverForwardsEvidenceLoss)
    ,("local Teams host cancels late forwarding and disposes idempotently", TeamsLocalMeetingDetectorTests.HostCancellationPreventsLateForwardingAndDisposeIsIdempotent)
    ,("silent Teams meeting and probe failure never auto stop", TeamsLocalMeetingDetectorTests.SilentMeetingAndProbeFailureNeverTriggerStop)
    ,("Teams process exit requires three polls and returned audio cancels stop", TeamsLocalMeetingDetectorTests.ProcessExitUsesBoundedFastStopAndReturnedAudioCancelsStop)
    ,("local Teams host forwards one bounded stop and a rejoin", TeamsLocalMeetingDetectorTests.HostForwardsOneBoundedStopAndARejoin)
    ...
    ,("window video lifecycle uses final native interval and audio fallback", AudioMvpTests.WindowVideoLifecyclePublishesFinalNativeIntervalAndFailsClosed)
    ,("window video can restart into non-overwriting segments", AudioMvpTests.WindowVideoCanRestartWithoutOverwritingSegments)
    ,("window video toggle and full stop serialize publication", AudioMvpTests.WindowVideoToggleAndFullStopAreSerialized)
    ...
    ,("video publication verifies MP4 and preserves audio fallback", SessionStorageTests.VideoPublicationVerifiesCompanionAndFallsBackToAudio)
    ...
    ,("Teams window capture catalog rejects unsafe targets", VideoCaptureTests.TeamsWindowPolicyRejectsUnsafeOrNonTeamsWindows)
    ,("Teams window capture identity does not persist title or path", VideoCaptureTests.WindowTitleIsNeverPartOfPersistedTargetData)
    ...
    ,("Teams automatic recorder cancels countdown when meeting evidence is unavailable", TeamsAutomaticRecordingControllerTests.UnavailableEvidenceCancelsCountdownWithoutStarting)
    ,("Teams automatic recorder stops only after confirmed meeting leave", TeamsAutomaticRecordingControllerTests.UnavailableEvidenceDoesNotStopRecordingButConfirmedLeaveDoes)
    ,("Teams automatic recorder ignores a same-generation late join", TeamsAutomaticRecordingControllerTests.SameGenerationLateJoinCannotReverseConfirmedLeave)
    ,("local Teams candidate does not mutate authoritative evidence revision", TeamsAutomaticRecordingControllerTests.LocalCandidateDoesNotMutateAuthoritativeEvidenceRevision)
    ,("returned local Teams evidence cancels local stop debounce", TeamsAutomaticRecordingControllerTests.LocalSignalReturnCancelsLocalStopDebounce)
